...
1.- Faut créer activer le service de la base de données. Pour le faire, dans votre répertoire du C:(disque dur) , vous allez vers "Program Files" ou "Program Files" (x86).
...
Dans le dossier frontend y'a que le frontend haha. Juste Angular, ce qu'il faut remarquer c'est le ObjectService. C'est lui qui fait les requêtes au serveur. Des fois j'ai écris Objet au lieu de Object (dans le code) parce que
le mot Object est déjà reservé.

tout ce qui est en dehors, c'est le serveur, il faut juste faire attention aux fichier appelés "Object" les autres sont d'un autre projet de Notes. C'est pour ca que si tu tappes "http://localhost:3000/" y'a ce projet qui s'ouvre. 
Mais le frontend est déjà programmé pour qu'il fasse de requêtes aux routes de notre projet.
